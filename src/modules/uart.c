...
LOG_MODULE_REGISTER(MODULE, CONFIG_UART_LOG_LEVEL);

#define UART_BUF_SIZE CONFIG_UART_BUF_SIZE

#define UART_SLAB_BLOCK_SIZE sizeof(struct uart_rx_buf)
#define UART_SLAB_BLOCK_COUNT (UART_DEVICE_COUNT * CONFIG_UART_BUF_COUNT)
...
static bool event_handler(const struct event_header *eh)
{
    ...
    if (is_uart_transmit_event(eh))
    {
        ...
        err = uart_tx_enqueue(event->dyndata.data, event->dyndata.size, event->dev_idx);
        if (err == -ENOMEM)
        {
            LOG_WRN("UART_%d overflow", event->dev_idx);
        }
        else if (err)
        {
...
EVENT_SUBSCRIBE_EARLY(MODULE, uart_transmit_event);
